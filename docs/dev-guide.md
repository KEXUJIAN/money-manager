...
| v0.4   | 2026-02 | 主键序贯重构、图表组件重载与 UI/UX 体验进阶                                    |
| v0.5   | 2026-02 | 修复账号清空Bug、实现交易表格独立时间筛选器、完善里程碑记录                    |
| v0.6   | 2026-03 | 原生双边转账、balanceOffset 平账、全面重构数据流向视图与导出                   |
| v0.6.1 | 2026-03 | DevDB中文化、流水编辑/删除功能、余额原子平账事务模型                           |
| v0.6.2 | 2026-03 | 路由懒加载规范、manualChunks 分包说明                                          |
...
1. 在 `src/routes/` 创建页面组件（必须 `export default`）
2. 在 `src/App.tsx` 中使用 `React.lazy(() => import("@/routes/xxx"))` 注册路由
3. 在 `src/layouts/AppLayout.tsx` 的 `navItems` 数组中添加导航项

> 所有路由页面均使用 `React.lazy` 懒加载，由外层 `<Suspense>` 统一处理加载态。
